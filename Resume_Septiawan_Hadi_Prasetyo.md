# RESUME — SEPTIAWAN HADI PRASETYO
**Software Engineer | Fullstack Developer | AI System Engineer**

📍 Ciwidey, Kabupaten Bandung, Jawa Barat, Indonesia  
✉️ [septiawanhadi38@gmail.com](mailto:septiawanhadi38@gmail.com) | 📞 [+62 821-1252-1412](https://wa.me/6282112521412)  
🔗 [LinkedIn](https://www.linkedin.com/in/septiawanhadip26/) | 🌐 [Google Skills Profile](https://www.skills.google/public_profiles/9a2937ac-b60e-4a01-af73-ff809612bbeb) | 💻 [Portofolio Web](https://septiawanhadi.github.io/septiawanhadip/)

---

### RINGKASAN EKSEKUTIF
Mahasiswa aktif Rekayasa Perangkat Lunak di Universitas Pendidikan Indonesia (UPI) semester 7 dengan pengalaman kerja magang industri sebagai **Software Engineer** di **Nix Offset** dan **Fullstack Developer** di **Diskominfo Kabupaten Bandung**. Memiliki spesialisasi dalam pengembangan aplikasi *End-to-End* (Web & Mobile React Native/Expo), perancangan antarmuka pengguna (UI/UX Figma), integrasi API sistem, serta implementasi *Artificial Intelligence* (Gen AI Azure, Prompt Engineering, Machine Learning). Terbiasa memecahkan masalah kompleks dengan pendekatan arsitektur data yang efisien dan orientasi produk berbasis *Human-Centered Design*.

---

### KEAHLIAN UTAMA (CORE SKILLS)
* **Bahasa Pemrograman**: Python, JavaScript, SQL, HTML5, CSS3.
* **Framework & Frontend/Mobile**: React Native, Expo, Next.js, React, RESTful API.
* **Backend & Database**: Fullstack Engineering, Relational Database (SQL), Server Logic, System Integration.
* **AI, Data & Cloud**: Generative AI (Microsoft Azure), Prompt Engineering, Machine Learning, Data Science, Google Cloud Platform (GCP), Microsoft Fabric.
* **UI/UX & Tools**: Figma, Wireframing, Prototyping, Human-Centered Design, Git/GitHub, VS Code.
* **Bahasa & Metodologi**: Bahasa Indonesia (Native), English (Professional Working), Riset Metodologis, Penulisan Ilmiah.

---

### PENGALAMAN KERJA / MAGANG
**Nix Offset** — Bandung, Indonesia  
*Software Engineer (Internship / Magang)* | *Juni 2026 – Agustus 2026*
* **Aplikasi Estimasi Potongan Bahan (Plano)**: Membangun perangkat lunak kalkulasi dan optimasi pemotongan lembaran bahan kemasan produk (plano) untuk meminimalkan sisa bahan baku (*waste*) dan meningkatkan akurasi estimasi biaya produksi.
* **Aplikasi Manajemen Office**: Mengembangkan aplikasi pengelolaan sistem internal kantor untuk menyederhanakan alur kerja operasional dan administrasi internal perusahaan.
* **Company Profile Website**: Merancang dan membangun situs web profil resmi perusahaan yang responsif untuk memperkuat citra digital dan jangkauan klien.

**Dinas Komunikasi dan Informatika (Diskominfo) Kabupaten Bandung** — Bandung, Indonesia  
*Fullstack Developer (Internship / Magang)* | *Februari 2026 – Juni 2026*
* **Pengembangan End-to-End**: Merancang serta mengimplementasikan fitur-fitur aplikasi web dan mobile layanan publik pemerintah daerah secara menyeluruh (Frontend & Backend).
* **Integrasi API & Database**: Melakukan integrasi API pihak ketiga dan standarisasi arsitektur database relasional untuk meningkatkan kecepatan pertukaran data.
* **Validasi & Deployment**: Berkolaborasi dengan tim infrastruktur IT untuk validasi kebutuhan teknis pengadaan sistem dan kesiapan deployment produk.

---

### PROYEK UNGGULAN
**UMKM Pintar Nusantara Berbasis AI** | *Lead Developer (Android App)* | [GitHub Link](https://github.com/septiawanhadi/UMKM-Pintar-Nusantara.git)
* Membangun aplikasi Android manajemen keuangan dan pelacakan kedaluwarsa stok UMKM menggunakan React Native dan Expo.
* Mengintegrasikan modul Kecerdasan Buatan (AI) untuk memberikan analisis tren inventaris dan rekomendasi bisnis otomatis.

**SafeTana AI — Tanggap Bencana & Kesehatan Digital** | *Fullstack AI Developer* | [Live Site](https://safetana.vercel.app/)
* Mengembangkan platform web tanggap bencana alam terintegrasi data *hazard real-time* menggunakan Next.js dan AI.
* Membangun modul *screening* kesehatan awal digital serta menyusun dokumentasi riset teknis akademis pendukung.

**Dataset Modul SPOK (NLP Research)** | *Publisher & Data Researcher* | [Kaggle Dataset](https://www.kaggle.com/datasets/septiawanhadip/modul-spok)
* Mengompilasi dan menerbitkan dataset terstruktur di Kaggle untuk klasifikasi tata bahasa Indonesia pola SPOK pada model pemrosesan bahasa alami (NLP).

---

### PENDIDIKAN
**Universitas Pendidikan Indonesia (UPI)** — Bandung, Indonesia  
*Sarjana Komputer (S.Kom) - Rekayasa Perangkat Lunak (S1)* | *2023 – Sekarang (Perkiraan Lulus: 2027)*
* Status: Mahasiswa Aktif Semester 7.
* Fokus Studi: Pengembangan Sistem Cerdas, Arsitektur Perangkat Lunak, Riset *Human-Centered Technology*, dan Penulisan Ilmiah.

**SMK Telekomunikasi Telesandi Bekasi** — Bekasi, Indonesia  
*Sekolah Menengah Kejuruan - Rekayasa Perangkat Lunak (RPL)* | *2021 – 2023*
* Mempelajari Pemrograman Berorientasi Objek (OOP), Basis Data Relasional, Web Development, dan Jaringan Komputer.

---

### SERTIFIKASI PROFESIONAL TERPILIH
* **Google Skills Public Profile (Silver League • 915 pts)** – *Google Skills*
* **Gen AI: Beyond the Chatbot (2026)** – *Google Skills*
* **Machine Learning Operations (MLOps) for Generative AI (2026)** – *Google Skills*
* **AI-Powered Performance Ads Certification (2026)** – *Skillshop / Accredible*
* **Google Analytics Certification (2026)** – *Skillshop / Accredible*
* **Membangun Aplikasi Gen AI dengan Microsoft Azure** – *Dicoding Indonesia*
* **Prompt Engineering untuk Software Developer** – *Dicoding Indonesia*
* **Belajar Machine Learning untuk Pemula** – *Dicoding Indonesia*
* **Data Science dengan Microsoft Fabric** – *Dicoding Indonesia*
* **Belajar Dasar Structured Query Language (SQL)** – *Dicoding Indonesia*
* **Belajar Dasar Google Cloud** – *Dicoding Indonesia*

---

### PUBLIKASI ILMIAH
* **Jurnal Ilmiah (2024)**: *"Penggunaan Teknologi Wearable: Untuk Optimalisasi Kebutuhan Atlet dan Masyarakat dalam Berolahraga"* — Jurnal Multidisiplin Inovatif. ([Link Jurnal](https://share.google/cIGWU3pLw7lsQMdRJ))
