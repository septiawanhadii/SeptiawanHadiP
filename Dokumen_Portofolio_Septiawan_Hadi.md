# DOKUMEN PORTOFOLIO
**Septiawan Hadi Prasetyo**  
*Junior Developer & Research | Fullstack Developer | UI/UX Designer*

---

## 👤 PROFIL SINGKAT
Saya adalah mahasiswa aktif program studi **Rekayasa Perangkat Lunak** di **Universitas Pendidikan Indonesia (UPI)** (angkatan 2023, perkiraan lulus 2027) yang berfokus pada pengembangan produk digital secara menyeluruh (fullstack development). Saya memiliki pengalaman magang sebagai **Software Engineer** di **Nix Offset** serta **Fullstack Developer** di **Dinas Komunikasi dan Informatika (Diskominfo) Kabupaten Bandung**. 

Keahlian utama saya terletak pada pembuatan aplikasi web dan mobile (React Native/Expo), perancangan UI/UX berbasis pendekatan *Human-Centered Design*, integrasi sistem pihak ketiga, serta penerapan kecerdasan buatan (Artificial Intelligence) dan analisis data.

---

## 🛠️ KEAHLIAN UTAMA & TEKNOLOGI

* **Desain & Frontend**: UI/UX Design, Figma, Prototyping, Human-Centered Design, React Native, Expo, HTML, CSS, JavaScript.
* **Backend & Database**: Fullstack Development, REST API, SQL, Python, IT Infrastructure, System Integration.
* **Kecerdasan Buatan & Data**: AI Integration, Machine Learning, Data Science, Prompt Engineering, Generative AI (Azure), Microsoft Fabric, Google Cloud Platform (GCP).
* **Riset & Akademis**: Riset Metodologis, Penulisan Karya Ilmiah, Financial Literacy.

---

## 💼 RIWAYAT PENGALAMAN KERJA / MAGANG

### 1. Nix Offset
* **Peran**: Software Engineer (Intern / Magang)
* **Durasi**: Juni 2026 – Agustus 2026
* **Tanggung Jawab & Tanggung Jawab Utama**:
  - **Company Profile Website**: Merancang dan membangun website profil perusahaan secara interaktif untuk branding dan informasi bisnis.
  - **Aplikasi Manajemen Office**: Mengembangkan aplikasi pengelolaan sistem internal kantor guna mengoptimalkan alur kerja administrasi dan operasional.
  - **Aplikasi Estimasi Potongan Bahan Kemasan (Plano)**: Membangun sistem kalkulasi dan estimasi optimasi pemotongan lembaran bahan kemasan produk (plano) untuk efisiensi bahan baku serta akurasi estimasi biaya produksi.

### 2. Dinas Komunikasi dan Informatika (Diskominfo) Kabupaten Bandung
* **Peran**: Fullstack Developer (Intern / Magang)
* **Durasi**: Februari 2026 – Juni 2026
* **Tanggung Jawab & Tanggung Jawab Utama**:
  - Merancang dan mengembangkan fitur-fitur aplikasi web dan mobile pemerintahan secara menyeluruh (Frontend & Backend).
  - Mengintegrasikan API pihak ketiga dan melakukan standarisasi struktur database relasional.
  - Berkolaborasi aktif dengan tim infrastruktur IT dan departemen terkait.

---

## 💻 RANGKUMAN PROYEK & KARYA

Berikut adalah daftar proyek pengembangan perangkat lunak yang telah saya rancang dan bangun:

### 1. UMKM Pintar Nusantara Berbasis AI (Android App)
* **Kategori**: Aplikasi Mobile (Android)
* **Teknologi**: React Native, Expo, AI Integration, Git/GitHub
* **Deskripsi**: Aplikasi khusus yang dirancang untuk membantu pengelolaan operasional UMKM secara modern. Memiliki fitur manajemen keuangan terstruktur, manajemen tanggal kedaluwarsa untuk kontrol inventaris barang (*expiry tracker*), serta modul kecerdasan buatan (AI) terintegrasi untuk menyajikan rekomendasi bisnis dan analisis data stok secara otomatis.
* **Link Tautan**: 
  - [Repositories (GitHub)](https://github.com/septiawanhadi/UMKM-Pintar-Nusantara.git)
  - [Build APK (Expo)](https://expo.dev/accounts/septiawanhadiprasetyos-team/projects/umkm-pintar-nusantara/builds/e6a815bb-4e88-4619-b85a-c0f51262aa45)

### 2. Project SafeTana AI
* **Kategori**: Aplikasi Web (Website App)
* **Teknologi**: React, Next.js, AI Integration, Real-time APIs
* **Deskripsi**: Platform tanggap bencana alam dan pemantauan kesehatan digital bertenaga kecerdasan buatan. Aplikasi ini menyinkronkan data hazard/bencana alam secara real-time untuk memberikan navigasi darurat dan pemeriksaan kesehatan awal digital berbasis kecerdasan buatan.
* **Link Tautan**:
  - [Buka Website](https://safetana.vercel.app/)
  - [Penulisan Riset Pendukung (Draft GDrive)](https://drive.google.com/drive/folders/1j_ugjEAoVJOomvJTNCuqJF-Y5jaPIse3?usp=drive_link)

### 3. Project SIDIK-TI
* **Kategori**: Aplikasi Web (Website App)
* **Teknologi**: PHP, Web Servers, Database Management, URL Masking Security
* **Deskripsi**: Sistem informasi manajemen infrastruktur teknologi informasi terintegrasi. Dirancang untuk standarisasi URL masking demi keamanan data serta validasi pengadaan internal departemen secara real-time.
* **Link Tautan**:
  - [Buka Website](http://sidik-ti.biz.id)

### 4. SagaHealth
* **Kategori**: Aplikasi Web & Riset UI/UX
* **Teknologi**: Figma, Web Development, Academic Research
* **Deskripsi**: Proyek riset komprehensif untuk aplikasi kesehatan digital. Proyek ini mencakup desain purwarupa UI/UX yang intuitif, repository kode dasar pengembangan web, serta dokumen/draft penulisan akademis pendukung.
* **Link Tautan**:
  - [Desain UI/UX (Figma)](https://www.figma.com/proto/uBN4ZNniZwLnaX6oNHdmtl/SAGAHEALTH?node-id=92-93&starting-point-node-id=122%3A8&t=Jkmv4hxAp6mAhRpS-1)
  - [Repository Pengembangan (GDrive)](https://drive.google.com/file/d/1S0AQlm2cxpxY5-FkRuZG34DgcVfOk1f3/view?usp=drive_link)
  - [Penulisan Pendukung (Draft GDrive)](https://drive.google.com/drive/folders/1qWlWOjfoRrfwGj52oq9Biy8J09JXzy8D?usp=drive_link)

### 5. Website Profile Sekolah SLB Raudhatul Zannah
* **Kategori**: Aplikasi Web (Website App)
* **Teknologi**: HTML, CSS, JavaScript, Responsive Web Design
* **Deskripsi**: Situs web profil resmi untuk sekolah SLB Raudhatul Zannah yang dikembangkan dengan antarmuka yang sangat aksesibel, ramah pengguna, serta responsif guna menyajikan profil lembaga, program pendidikan khusus, serta galeri kegiatan.
* **Link Tautan**:
  - [Buka Website](https://slbraudhatulzannah.biz.id)

### 6. Project Event To
* **Kategori**: Aplikasi Web (Website App)
* **Teknologi**: Web Development, Database Management, Git/GitHub
* **Deskripsi**: Proyek pengembangan repository kode sumber aplikasi manajemen acara/event terpadu.
* **Link Tautan**:
  - [Repositories (GitHub)](https://github.com/nabilaaabill/evenTO.git)

### 7. Project Noto
* **Kategori**: Aplikasi Desktop (Desktop App)
* **Teknologi**: Java/Python, GUI Desktop, Local Database
* **Deskripsi**: Proyek pembuatan aplikasi desktop lokal untuk pencatatan harian, pengelolaan tugas, dan manajemen catatan pintar (*notes management*).
* **Link Tautan**:
  - [Repositories (GitHub)](https://github.com/gilangrp/noto.git)

### 8. Dataset Modul SPOK (Natural Language Processing)
* **Kategori**: Dataset & NLP Research
* **Teknologi**: Python, Structured Data, Kaggle Platform
* **Deskripsi**: Paket data terstruktur yang dikompilasi untuk kebutuhan penelitian klasifikasi tata bahasa Indonesia dengan pola SPOK (Subjek, Predikat, Objek, Keterangan). Berguna sebagai data latih model NLP (*Natural Language Processing*).
* **Link Tautan**:
  - [Dataset (Kaggle)](https://www.kaggle.com/datasets/septiawanhadip/modul-spok)

---

## 🎨 UI/UX CASE STUDY (PORTFOLIO DESAIN)

Selain pemrograman, saya aktif merancang alur interaksi antarmuka (*user journey*) yang optimal:

* **UI/UX &mdash; Redesign InDrive App**: Studi kasus desain ulang antarmuka aplikasi pemesanan transportasi InDrive di Figma untuk meningkatkan kemudahan transaksi pengguna (*usability*).
  - [File Desain (Figma)](https://www.figma.com/proto/PIPzwMaHh9h9vjC0srGamU/IMK-kelompok-2?node-id=138-896&starting-point-node-id=211%3A9861&t=c6Ioc1MLdg581gMo-1)
* **UI/UX &mdash; Project AgrowUp App**: Rancangan purwarupa UI/UX aplikasi cerdas sektor pertanian (*Agrotech*) untuk membantu petani memonitor komoditas produk.
  - [File Desain (Figma)](https://www.figma.com/proto/2qRIobYWNeiQNXX8dXi2ZX/Project-PKM-Kodamas?node-id=81-14&t=MWVmtSi1anjsfoML-1)

---

## 📄 JURNAL & ARTIKEL PUBLIKASI (RISET)

Berikut adalah daftar publikasi ilmiah dan tulisan akademis/artikel yang telah saya buat:

1. **Jurnal Ilmiah (2024)**: *"Penggunaan Teknologi Wearable: Untuk Optimalisasi Kebutuhan Atlet dan Masyarakat dalam Berolahraga"* &mdash; Diterbitkan pada Jurnal Multidisiplin Inovatif.
   - [Link Jurnal](https://share.google/cIGWU3pLw7lsQMdRJ)
2. **Artikel Publikasi (2025)**: *"Audiensi Pemilu BEM Kampus UPI di Cibiru"* &mdash; Diterbitkan oleh UPI Cibiru.
   - [Link Artikel](https://kd-cibiru.upi.edu/index.php/component/content/article/audiensi-pemilu-bem-kampus-upi-di-cibiru?catid=28&Itemid=101)
3. **Artikel Publikasi (2025)**: *"Pemilu BEM Tahun 2025 di Kampus UPI Cibiru: MPU, DPPU, dan KPU Siap Jalankan Tugas"* &mdash; Diterbitkan oleh UPI Cibiru.
   - [Link Artikel](https://kd-cibiru.upi.edu/index.php/component/content/article/pemilu-bem-tahun-2025-di-kampus-upi-cibiru-mpu-dppu-dan-kpu-siap-jalankan-tugas?catid=28&Itemid=101)

---

## 🏅 SERTIFIKASI PROFESIONAL

1. **AI-Powered Performance Ads Certification (2026)** – *Skillshop* ([Verifikasi](https://www.credential.net/0b6288c1-3701-4dfa-8b87-9e12ca923796))
2. **Google Analytics Certification (2026)** – *Skillshop* ([Verifikasi](https://www.credential.net/fed13720-838c-4f0e-93cc-63edadecc08c))
3. Saya aktif memperbarui keahlian melalui sertifikasi kredibel dari Dicoding Indonesia (Partner Google/Microsoft):
4. **Membangun Aplikasi Gen AI dengan Microsoft Azure** (AI Generatif, Cloud)
5. **Prompt Engineering untuk Software Developer** (AI Interaction, Rekayasa Prompt)
6. **Belajar Machine Learning untuk Pemula** (Kecerdasan Buatan, ML Models)
7. **Data Science dengan Microsoft Fabric** (Analisis Data, ETL)
8. **Belajar Data Science** (Statistika, Data Processing)
9. **Belajar Fundamental Pemrosesan Data** (Data Engineering)
10. **Belajar Dasar Structured Query Language (SQL)** (Manajemen Basis Data Relasional)
11. **Belajar Dasar Google Cloud** (Cloud Computing, GCP Services)
12. **Belajar Dasar AI** (Konsep Dasar Kecerdasan Buatan)
13. **Memulai Pemrograman dengan Python** (Logika OOP, Python Basics)
14. **Financial Literacy** (Literasi Keuangan)
14. ---

## 📞 KONTAK & SALURAN KOMUNIKASI

Hubungi saya untuk mendiskusikan peluang kolaborasi, riset, atau pengembangan proyek:

* **Email**: [septiawanhadi38@gmail.com](mailto:septiawanhadi38@gmail.com)
* **WhatsApp**: [+62 821-1252-1412](https://wa.me/6282112521412?text=Halo%20septi%20saya%20melihat%20anda%20dari%20portofolio%20anda%2C%20saya%20ingin%20mengajak%20berbicara%20serta%20berdiskusi%20sebentar%20apakah%20berkenan%3F)
* **Instagram**: [@sptwnhdprsty_](https://www.instagram.com/sptwnhdprsty_)
* **LinkedIn**: [LinkedIn/septiawanhadip26](https://www.linkedin.com/in/septiawanhadip26/)
